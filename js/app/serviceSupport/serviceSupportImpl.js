class serviceSupportImpl {
  ...
  setCurrentEpisode (episodeId) {
    const self = this
    if (self.selectedEpisode != null && self.selectedEpisode.id === episodeId) {
      return true
    }

    const episode = _.find(self.episodeList, (episode) => {
      return episode.id === episodeId
    })

    if (episode) {
      self.selectedEpisode = { id: episode.id, title: episode.title, url: episode.url, players: [] }
